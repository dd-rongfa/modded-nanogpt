...
class CausalSelfAttentionNoRope(nn.Module):
...
class Block2(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.attn = CausalSelfAttentionNoRope(config)
        self.mlp = MLP(config)
        self.lambdas = nn.Parameter(torch.tensor([1., 0.]))
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([Block(config) if i % 2 == 0 else Block2(config) for i in range(config.n_layer)]),
        ))
...
class Hyperparameters:
    ...
    batch_size : int = 1*16 # batch size, in sequences, across all devices
